...
if(isset($_POST['submit'])){
    ...
    $stmt=$dbcon->prepare('update webdesigner set sname=:sname,semail=:semail,sphone=:sphone,smessage=:smessage where id=:id');
    ...
    if($eq){
      echo "<script>alert('updated data sucess....');
          location.href='delete_edit_webdesigner_courses.php';
          </script>";
      } else {
...
if($opr=='delete'){
    $stmt=$dbcon->prepare('delete from webdesigner where id=:id');
    $stmt->bindParam(':id',$id);
    $eq=$stmt->execute();
    if($eq){
...
<div class="container-fluid breadcrumbs">
                <div class=" container">
                    <div class="col-md-12">
                        <div class="col-md-6">
                        <h1 class="page-title p-2">Webdesigner_Courses_Register</h1>
                        </div>
                    </div>
                    <div class="col-md-6 py-2">
                        <ul>
                            ...
                            <li  class=" text-warning">Webdesigner-Courses_Register // </li>
                        </ul>
                        </div>
                </div>
                </div>
...
<section class="container py-5">
    <?php
     $stmt=$dbcon->prepare('select * from webdesigner where id=:id');
     $stmt->bindParam(':id',$id);
     $eq=$stmt->execute();
