...
if(isset($_POST['submit'])){
    ...
    $stmt=$dbcon->prepare('update phpmysql set sname=:sname,semail=:semail,sphone=:sphone,smessage=:smessage where id=:id');
    ...
    if($eq){
      echo "<script>alert('updated data sucess....');
          location.href='delete_edit_php_mysql_courses.php';
          </script>";
      } else {
...
if($opr=='delete'){
    $stmt=$dbcon->prepare('delete from phpmysql where id=:id');
    $stmt->bindParam(':id',$id);
    $eq=$stmt->execute();
    if($eq){
...
<section class="container py-5">
    <?php
     $stmt=$dbcon->prepare('select * from phpmysql where id=:id');
     $stmt->bindParam(':id',$id);
     $eq=$stmt->execute();
